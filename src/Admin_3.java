...
class Admin_3 {
	...
	public Admin_3() {
		JFrame frm=new JFrame("药房系统");
		frm.setVisible(true);
		frm.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frm.setBounds(100, 100, 774, 475);
		...
		panel.setBounds(20, 35, 379, 298);
		contentPane.add(panel);
		panel.setLayout(null);
		
		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(0, 0, 379, 188);
		...
		lblNewLabel_3.setBounds(0, 134, 54, 24);
		panel_1.add(lblNewLabel_3);
		
		JLabel lblNewLabel_4 = new JLabel("\u603B\u91CF");
		...
		textField.setBounds(57, 37, 66, 21);
		panel_1.add(textField);
		textField.setColumns(10);
		textField_1 = new JTextField();
		textField_1.setBounds(57, 84, 66, 21);
		panel_1.add(textField_1);
		textField_1.setColumns(10);
		
		textField_2 = new JTextField();
		textField_2.setBounds(57, 134, 66, 21);
		panel_1.add(textField_2);
		textField_2.setColumns(10);
		
		textField_3 = new JTextField();
		textField_3.setBounds(57, 179, 66, 21);
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(table.getSelectedRow()!=-1)
				{
					...
					String iden=table.getValueAt(row, 0).toString();
					String MName=table.getValueAt(row, 1).toString();
					int price=Integer.parseInt(table.getValueAt(row, 2).toString());
					int num=Integer.parseInt(table.getValueAt(row, 3).toString());
					lblNewLabel.setText("删除");
					textField.setText(iden);
		...
		btnNewButton_4.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(lblNewLabel.getText().equals("更改")){
					...
					String sql="update medicine set MName='"+textField_1.getText()+"', price='"+Integer.parseInt(textField_2.getText())+"', num='"+Integer.parseInt(textField_3.getText())+"'where iden='"+textField.getText()+"'";
					try {
				...
				if(lblNewLabel.getText().equals("删除")){
					...
					String sql="delete from medicine where iden='"+textField.getText()+"'";
					try {
						st.executeUpdate(sql);
					} catch (SQLException e) {
						// TODO Auto-generated catch block
				...
				if(lblNewLabel.getText().equals("增加")){
				    ...
				    Statement st=null;
		         	...
		         	String sql_1="select * from medicine where iden='"+textField.getText()+"'";
		         	try {
		         		ResultSet rs=st.executeQuery(sql_1);
		         		if(rs.next()){
		         			lblNewLabel_6.setText("编号已存在");
		         			try {
								st.close();
								con.close();
							} catch (SQLException e) {
						    ...
						    Statement s=null;
				         	...
				         	String sql="insert into medicine values('"+textField.getText()+"','"+textField_1.getText()+"','"+Integer.parseInt(textField_2.getText())+"','"+Integer.parseInt(textField_3.getText())+"')";
							try {
								s.executeUpdate(sql);
							} catch (SQLException e) {
								// TODO Auto-generated catch block
